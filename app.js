...
function SayGoodBye(req,res,intent) {
		  send_alexa_response(res, 'Good Bye!', 'GoodBye', 'Say Bye', 'GoodBye!', true);
...
function PostCaseFollowUp(req,res,intent) {
	...
	function(err,result) {
		if(err) {
		  ...
		  console.log(result);	
		  send_alexa_response(res, 'I have posted a request to follow up with the customer on the chatter feed for this case. Good Bye!', 'Case Summary', 'Submitted Follow up request.', 'I have posted a request to follow up with the customer on the chatter feed for this case. Good Bye!', true);
		}
	});
}
